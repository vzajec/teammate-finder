...
export default {
  ...
  methods: {
        ...
        fetchMessagesDota(){
            db.collection('chat').doc("dota").collection("messages").orderBy('createdAt').onSnapshot((querySnapshot)=>{
                let allMessages=[];
                querySnapshot.forEach(doc=>{
                    allMessages.push(doc.data())
                })
                this.messagesdota=allMessages;
                setTimeout(()=>{
                  this.scrollToBottom();
                },1000);
            })
        ...
        fetchMessagesCsgo(){
            db.collection('chat').doc("csgo").collection("messages").orderBy('createdAt').onSnapshot((querySnapshot)=>{
                let allMessages=[];
                querySnapshot.forEach(doc=>{
                    allMessages.push(doc.data())
                })
                this.messagescsgo=allMessages;
                setTimeout(()=>{
                  this.scrollToBottom();
                },1000);
            })
        ...
        fetchMessagesLol(){
            db.collection('chat').doc("lol").collection("messages").orderBy('createdAt').onSnapshot((querySnapshot)=>{
                let allMessages=[];
                querySnapshot.forEach(doc=>{
                    allMessages.push(doc.data())
                })
                this.messageslol=allMessages;
                setTimeout(()=>{
                  this.scrollToBottom();
                },1000);
            })
